...
document.addEventListener('DOMContentLoaded', () => {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, ([tab]) => {
    ...
    // Not on YouTube — restore saved URL + cached languages
    chrome.storage.local.get(['savedUrl', 'cachedLanguages'], ({ savedUrl, cachedLanguages }) => {
      if (savedUrl) {
        ...
        if (vid) {
          lastVideoId = vid;
          setControlsEnabled(true);
          // Use cached languages if same video, otherwise fetch fresh
          if (cachedLanguages?.videoId === vid && cachedLanguages.languages?.length) {
            renderLanguageDropdown(cachedLanguages.languages);
          } else {
            loadLanguages(vid);
          }
        }
...
async function handleDownload() {
  ...
  try {
    ...
    chrome.runtime.sendMessage(
      { type: 'FETCH_TRANSCRIPT', videoId, lang, format, timestamps, tabId: currentTabId },
      (res) => {
        elBtnDownload.disabled = false;
        elBtnDownload.className = 'grass-btn-primary w-full mt-4 text-xs';
        elBtnDownload.style.pointerEvents = '';
...
function renderLanguageDropdown(languages) {
  ...
  chrome.storage.local.get(['savedLang', 'savedLangLabel'], ({ savedLang, savedLangLabel }) => {
    if (savedLang && savedLangLabel) {
      const exists = languages.some((l) => l.code === savedLang) || savedLang === 'auto';
      if (exists) {
...
function loadLanguages(videoId) {
  ...
  chrome.runtime.sendMessage({ type: 'LIST_LANGUAGES', videoId, tabId: currentTabId }, (res) => {
    ...
    // Cache languages for this video
    chrome.storage.local.set({ cachedLanguages: { videoId, languages: res.languages } });

    renderLanguageDropdown(res.languages);
  });
}
...
function toast(text, isError) {
  ...
  elToast.className = `fixed top-3 left-1/2 -translate-x-1/2 z-50 text-xs font-bold px-4 py-2 rounded-full tracking-wide shadow-hard ${
    isError ? 'bg-red-500 text-white' : 'bg-gray-800 text-lime'
  }`;
